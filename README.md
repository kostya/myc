...
To get clean measurements, I saved all IR files generated by mycc: `mycc file.c d > file.myc` into the [LangArena/myc](https://github.com/kostya/LangArena/tree/master/myc) directory. This removes libclang overhead and double IR conversion (current mycc pain points) from the measurements. Essentially, this is pure Myc IR -> binary compilation time, without the C parser for LangArena benchmark.

If you write a good fast parser for C, it would add ~300-400ms to the compilation time (based on cproc results). All build time measurements above are single-threaded, without caches.
